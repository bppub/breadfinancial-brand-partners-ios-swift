...
private final class SVGImageRenderer: NSObject, WKNavigationDelegate {
  ...
  func render(data: Data, completion: @escaping (UIImage?) -> Void) {
      ...
      // Strip any XML prolog / DOCTYPE (e.g. `<?xml …?>` and
      // `<!DOCTYPE … [ … ]>`) that precedes the `<svg>` element. When the SVG
      // is embedded in an HTML document these are parsed as text and can leak
      // stray characters such as `]>` in front of the rendered logo.
      var svgString = String(data: data, encoding: .utf8) ?? ""
      if let svgRange = svgString.range(of: "<svg", options: .caseInsensitive) {
          svgString = String(svgString[svgRange.lowerBound...])
      }
      let html = """
      <html>
      <head>
